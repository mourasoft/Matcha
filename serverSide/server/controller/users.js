...
async function isConfirmed(user_id) {
  ...
  if (res2.data.length && res2.data[0].con_type != "ResetPasswd") return 1;
  else if (res2.data.length) await ConfModel.dropConf(res2.data[0].con_id);
  return 0;
}

let is_blocked = async (userid, otherid) => {
  let isblockd = await require("../model/blocks").getBlockedWhere(
    "(`user_id` = ? AND `to_id` = ?) OR (`user_id` = ? AND `to_id` = ?)",
    "NULL",
    [otherid, userid, userid, otherid]
  );
  if (isblockd.length) return true;
  return false;
};

async function LoginisExist(login) {
  let result = await UsersModel.getUsersWhere("`login` LIKE ?", columns, [
    login,
  ]);
  return result;
}
...
exports.signin = async (req, res) => {
  ...
  result = await UsersModel.getUsersWhere(
    "`login` LIKE ?",
...
exports.get_infos = async (req, res) => {
  ...
  if (login == undefined || login == "" || /[^A-Za-z0-9]+/.test(login))
    return res.status(200).json({
      success: false,
      error: true,
  ...
  result = await UsersModel.getUsersWhere("`user_id` = ?", clmns, [
...
exports.updateAccount = async (req, res) => {
  ...
  if (
    ...
    update_columns +=
      update_columns == "" ? "`first_name`=?" : ", `first_name`=?";
  ...
  if (
    ...
    update_columns +=
      update_columns == "" ? "`last_name`=?" : ", `last_name`=?";
  ...
  if (update_columns.search("email") > -1) {
    const result = await ConfModel.getConfWhere("`user_id` = ?", [
      req.jwt.user_id,
    ]);
    if (!result.data.length) {
      const key = await generateValKey();
      await ConfModel.CreateConf({
        con_key: key,
      ...
      mail.validateEmail(
        {
          fname: req.jwt.first_name,
          lname: req.jwt.first_name,
          login: req.jwt.login,
...
exports.changePasswd = async (req, res) => {
  ...
  result = await ConfModel.getConfWhere(
    "`user_id` = ? AND `con_key` = ? AND `con_type` = 3",
    [user_id, key]
  );
  if (!result.data.length)
    return res.status(200).json({
  ...
  await UsersModel.updateAccount("`passwd`=?", [
...
exports.updatePassword = async (req, res) => {
  let result = await UsersModel.getUsersWhere("`user_id` = ?", "`passwd`", [
    req.jwt.user_id,
  ]);
  bcrypt.compare(
    req.body.oldpasswd,
    result.data[0].passwd,
    async (err, succ) => {
      if (succ) {
        await UsersModel.updateAccount("`passwd`=?", [
          bcrypt.hashSync(req.body.npasswd, 10),
          req.jwt.user_id,
        ]);
        return res.status(200).json({
          error: false,
...
exports.resetPassword = async (req, res) => {
  ...
  let result = await UsersModel.getUsersWhere(
    "`email` LIKE ?",
  ...
  if (result.data.length)
    return res.status(200).json({
      ...
      success: false,
      message:
        "fetch your email, already send a validation url of " +
        result.data[0].con_type,
    });
  let key = await generateValKey();
  await ConfModel.CreateConf({ user_id: user_id, con_key: key, con_type: 3 });
  mail.resetPasswd(
    { login: login, lname: last_name, fname: first_name, email: email },
    key
  );
  return res.status(200).json({
    error: false,
