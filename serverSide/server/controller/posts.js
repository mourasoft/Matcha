...
let getUimg = async (userid) => {
  let dt = await imagesmodel.getImgsWhere(
    "`user_id` = ? ORDER BY `created_dat` DESC LIMIT 1",
    [userid]
  );
...
let getUinfos = async (userid) => {
  let sql =
    "SELECT `us`.`login`, `us`.`first_name`, `us`.`last_name`, " +
...
let isCompletedProfil = async (userid) => {
  let sql =
    "SELECT NULL FROM `users_infos` , `users_tags` WHERE (`users_infos`.`user_id` = ? AND `users_tags`.`user_id` = ?)";
...
let makeTagsFilterQuery = async (tags) => {
  ...
  let columns =
    "(SELECT COUNT(*) FROM `users_tags` WHERE (`user_id` = `users`.`user_id` ";
...
let sendposition = async (userid) => {
  ...
  let data = await rp
    ...
    .then(async (result) => {
      await require("../model/position").setposition(
        userid,
...
exports.getPosts = async (req, res) => {
  ...
  if (!data.length) await sendposition(req.jwt.user_id);
  data = await positionmodel.getposition(req.jwt.user_id);
  let userlat = data[0].latitude,
    userlong = data[0].longitude;
  let sexpref = await query(
    "SELECT `sex_pref` FROM `users_infos` WHERE `user_id` = ? LIMIT 1",
    [req.jwt.user_id]
  );
  sexpref = sexpref[0].sex_pref;
  // 'female', 'male', 'both'
  if (limit == undefined || /[^0-9]+/.test(limit)) limit = 0;
  let query_filter =
    "`age` >= " +
    minAge +
    " AND `age` <= " +
  ...
  let sql =
    ...
    filterbysexpref(sexpref, req.query.flag) +
    ...
    query_filter +
    "AND `interstags` >= " +
    JSON.parse(intereststags).length +
    " ORDER BY " +
    sortType +
    " LIMIT " +
    limit +
  ...
  while (i < data.length) {
    ...
    data[i].intersting =
      sexpref == "both" ? 1 : sexpref == data[i].infos[0].gendre ? 1 : 0;
    i++;
  }
  return res.status(200).json({
    error: false,
