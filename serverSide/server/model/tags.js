...
exports.Insert_inexistenttags = async (tags) => {
  ...
  while (i < tags.length) {
    tags[i] = tags[i].trim();
    if (tags[i] !== "" && tags[i].length <= 5) {
      let res = await query(sql_select, [tags[i]]);
      if (!res.length) await query(sql_insert, [[[tags[i]]]]);
    }
    i++;
  }
  return true;
...
exports.linktags = async (tags, userid) => {
  ...
  var sql_select2 =
    "SELECT * FROM `users_tags` WHERE (`user_id` = ? AND `tags_id` = ?)";
  var sql_insert =
    "INSERT INTO `users_tags` (`user_id`, `tags_id`, `state`) VALUES ?";
  let update_stm =
    "UPDATE `users_tags` set `state`=1, `modified_dat`=CURRENT_TIMESTAMP WHERE (`utags_id` = ?)";

  var i = 0;
  await query("UPDATE `users_tags` set `state`=2 WHERE `user_id` = ?", [
    userid,
  ]);
  while (i < tags.length) {
    tags[i] = tags[i].trim();
...
exports.gettags_where = async (where, columns, vars) => {
  var sql =
    "SELECT " + columns + " FROM `users_tags`, `alltags` WHERE " + where;
