...
let Changelocation = async (userid, city) => {
  await rp
    .get({
      uri:
        "http://api.positionstack.com/v1/forward?access_key=d696007b4568b87117e5caa046366e5e&query=" +
        city,
      json: true,
    })
    .then(async (result) => {
      if (
        ...
        result.data.length &&
        result.data[0].longitude != null &&
        result.data[0].latitude != null
      ) {
        await require("../model/position").setposition(
          userid,
      ...
      } else await positionbyip(userid);
    })
    .catch((err) => {
      console.log(err);
    });
    ...
    if (!err) {
      ...
      while (i < 400) {
        while (1) {
          ...
          dt = await usersmodel.getUsersWhere(
            "`login` like ? LIMIT 1",
            "NULL",
            [login]
          );
          if (!dt.data.length) break;
        }
          ...
          async (err, data) => {
            if (!err) {
              let imgname =
                ...
                ".jpg";
              fs.writeFile(
                "./public/users_imgs/" + imgname,
                data,
                async (err) => {
                  if (err) throw err;
                  else await imgsmodel.insert_img(infos.user_id, imgname);
                }
              );
