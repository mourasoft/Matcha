...
function gendreIsvalide(gendre) {
  if (gendre > 0 && gendre < 3) return 0;
  return 1;
}

function cityIsvalide(city) {
  if (/[^A-Za-z\s\W]+/.test(city) || city == "") return 1;
  return 0;
}

function descIsvalide(desc) {
  if (!desc.length) return 1;
  return 0;
}
...
function bdayISvalide(birthday) {
  if (
    !/^([0-9]{2}[\-]{1}[0-9]{2}[\-]{1}[0-9]{4})$/.test(birthday) &&
...
function sexprefIsvalide(sexpref) {
  if (sexpref > 0 && sexpref <= 3) return 0;
  return 1;
}
...
let InfosExists = async (userid) => {
  ...
  if (!result.data.length) return 1;
  return 0;
};
...
exports.updateinfo = async (req, res, next) => {
  ...
  if (
    ...
    req.body.birthday != "" &&
    bdayISvalide(req.body.birthday)
  )
    return res.status(200).json({
      error: true,
      success: false,
  ...
  if (
    req.body.gendre != undefined &&
    req.body.gendre != "" &&
    gendreIsvalide(req.body.gendre)
  )
    return res.status(200).json({
      error: true,
      success: false,
  ...
  if (
    req.body.sexpref != undefined &&
    req.body.sexpref != "" &&
    sexprefIsvalide(req.body.sexpref)
  )
    return res.status(200).json({
      error: true,
      success: false,
