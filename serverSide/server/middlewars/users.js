...
function loginisValide(login, status) {
  if (/[^a-zA-Z0-9]+/.test(login)) {
    if (status != null) status.error = "login contain invalid caractere";
    return 1;
  }
  if (login.length < 4) {
    if (status != null) status.error = "login is too short";
    return 1;
  }
  return 0;
}
...
function passwdisValide(passwd, status) {
  if (
    ...
    if (status)
      status.error =
        "password must contain UpperCase, LowerCase, Numbers, Special Caractere";
    return 2;
  }
  if (passwd.length < 8) {
    if (status) status.error = "password must contain 8 caractere";
    return 1;
  }
  return 0;
}
...
function fnameisValide(fname, status) {
  if (/[^A-Za-z]/.test(fname)) {
    if (status != null) status.error = "first name has invalid caractere";
    return 1;
  }
  return 0;
}
...
function lnameisValide(lname, status) {
  if (!/^([A-Za-z]+[\ ]{0,1}[A-Za-z]*)$/.test(lname)) {
    if (status != null) status.error = "last name has invalid caractere";
    return 1;
  }
  return 0;
}
...
async function loginisUnique(login, status) {
  var result = await UsersModel.getUsersWhere(
    '`login` LIKE "' + login + '"',
    "`login`"
  );
  ...
  if (result.data.length) {
    if (status != null) status.error = "login already used";
    return 1;
  }
  return 0;
}
...
async function emailisUnique(email, status) {
  ...
  if (result.data.length) {
    if (status != null) status.error = "email already used";
    return 1;
  }
  return 0;
}
...
async function checkData(Data, status) {
  ...
  if (
    fnameisValide(Data.fname, status) ||
...
exports.updateAccount = async (req, res, next) => {
  if (
    ...
    (loginisValide(req.body.login, null) ||
  ...
  if (
    ...
    (emailisValide(req.body.email, null) ||
  ...
  if (
    ...
    fnameisValide(req.body.first_name, null)
  )
    return res.status(200).json({
      error: true,
      success: false,
  ...
  if (
    ...
    lnameisValide(req.body.last_name, null)
  )
    return res.status(200).json({
      error: true,
      success: false,
  ...
  if (npasswd == undefined || (errnmr = passwdisValide(npasswd)))
    return res.status(200).json({
      ...
      message:
        errnmr == 2
          ? "password must contain UpperCase, LowerCase, Numbers, Special Caractere"
          : "password must contain 8 caractere",
    });
  next();
};
